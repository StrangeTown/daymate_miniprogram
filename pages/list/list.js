...
const { getIconUrlByIcon } = require("../../utils/icons");
...
Page({
	...
	loadEvents(pageNum = 1, isLoadMore = false) {
		...
		fetchPagedEventList(
			...
			(data) => {
				const newEvents = (data.list || []).map((event) => ({
					...event,
					daysLeft: this.calculateDaysLeft(event.eventDate.split("T")[0]),
					image: getIconUrlByIcon(event.icon) || "",
	...
	onReachBottom() {
		console.log(
			"onReachBottom triggered, hasMore:",
			this.data.hasMore,
			"loading:",
			this.data.loading
		);
	...
	deleteEventById(eventId) {
		...
		deleteEvent(
			...
			(data) => {
				...
				const updatedEvents = this.data.events.filter(
